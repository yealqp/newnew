//! OpenGate gateway as a library: the `gateway` binary and the Tauri desktop
//! app both build the same router / state through this crate root.
...
/// Build the complete application router (admin API + relay + health, plus the
/// embedded web UI when the `embed-frontend` feature is enabled).
pub fn build_router(state: AppState) -> Router {
    ...
    #[cfg(feature = "embed-frontend")]
    let app = app.fallback(webui::static_handler);
